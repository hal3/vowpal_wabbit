...
UNAME := $(shell uname)
LIBS = -l boost_program_options -l pthread -l boost_iostreams -l Cntk.PerformanceProfiler-2.0 -l Cntk.Math-2.0 -l Cntk.Core-2.0
BOOST_INCLUDE = -I /usr/include
BOOST_LIBRARY = -L /usr/local/lib -L /usr/lib
CNTK_ROOT = /home/hal/projects/CNTK
CNTK_INCLUDE = -I $(CNTK_ROOT)/Source/CNTKv2LibraryDll/API
CNTK_LIBRARY = -L $(CNTK_ROOT)/lib
NPROCS := 1
...
FLAGS = -std=c++14 $(CFLAGS) $(LDFLAGS) $(ARCH) $(WARN_FLAGS) $(OPTIM_FLAGS) -D_FILE_OFFSET_BITS=64 $(BOOST_INCLUDE) $(CNTK_INCLUDE) $(JSON_INCLUDE) -fPIC #-DVW_LDA_NO_SSE

# for profiling -- note that it needs to be gcc
#FLAGS = -std=c++14 $(CFLAGS) $(LDFLAGS) $(ARCH) $(WARN_FLAGS) -O2 -fno-strict-aliasing -D_FILE_OFFSET_BITS=64 $(BOOST_INCLUDE) $(CNTK_INCLUDE) $(JSON_INCLUDE) -pg  -fPIC
#CXX = g++

# for valgrind / gdb debugging
#FLAGS = -std=c++14 $(CFLAGS) $(LDFLAGS) $(ARCH) $(WARN_FLAGS) -D_FILE_OFFSET_BITS=64 $(BOOST_INCLUDE) $(CNTK_INCLUDE) $(JSON_INCLUDE) -g -O0  -fPIC

# for valgrind profiling: run 'valgrind --tool=callgrind PROGRAM' then 'callgrind_annotate --tree=both --inclusive=yes'
#FLAGS = -std=c++14 $(CFLAGS) $(LDFLAGS) -Wall $(ARCH) -ffast-math -D_FILE_OFFSET_BITS=64 $(BOOST_INCLUDE) $(CNTK_INCLUDE) $(JSON_INCLUDE)  -g -fomit-frame-pointer -ffast-math -fno-strict-aliasing  -fPIC
